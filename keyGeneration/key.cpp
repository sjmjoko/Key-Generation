...
int main()
{
	...
	while (cycle < rows)
	{
		for (int i = 0; i < 32; i++)
		{
			tempTextLeft.push_back(textBin[cycle][i]);
		}
		for (int i = 32; i < textBin[cycle].size(); i++)
		{
			tempTextRight.push_back(textBin[cycle][i]);
		}
		for (int i = 0; i < 16; i++)
		{
			expansionTable(rightText, tempTextRight);
			tempTextRight.clear();
			XOR(encryptionKey, rightText, cipherText, i);
			rightText.clear();
			to2D(temp, cipherText);
			cipherText.clear();
			sBox(temp, tempTextRight);
			pBox(tempTextRight, rightText);
			tempTextRight.clear();
			xor(cipherText,tempTextLeft,rightText);
			passover(cipher, cipherText);
			tempTextLeft.clear();
			passover(tempTextLeft, cipher);
			passover(tempTextRight, cipher);
			cipher.clear();
		}
